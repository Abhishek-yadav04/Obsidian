...
func Test941310(t *testing.T) {
	// Go regex only supports UTF-8 strings. Matching non-UTF-8 input would require
	// escaping to ASCII before matching, which is not worth the performance cost
	// since Go HTTP libraries generally don't support non-UTF-8 anyway.
